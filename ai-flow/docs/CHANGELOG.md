...
## [2026-09-22] - ai-flow: устойчивый парсер Depends on и явные параллельные batch-и (из рабочего проекта)
- Парсер (`run_tasks.py`): общий `_parse_refs` для `Depends on:` / `Зависит от:` и нового
  `Parallel with:` / `Параллельно с:` — читает перенос списка строкой с отступом, игнорирует пояснения
  в скобках, бэктики, суффикс `.md` и текст после тире; токен без `_`/`-` считается прозой. Раньше
  каждый из этих случаев молча блокировал очередь или (перенос строки) запускал задачу раньше срока.
- Параллельные batch-и: взаимно и полностью объявленные в `Parallel with:` готовые задачи одной фичи
  выполняются атомарно — воркеры в отдельных detached git worktree, коммиты cherry-pick-ом в порядке
  имён в интеграционный worktree, один проход интеграции, затем fast-forward; при сбое основное дерево
  не меняется, готовый batch сохраняется в `ai-flow/recover-<задача>`. Воркер не трогает общие доки
  (файл правил, журнал, спеки, памяти, Markdown задач и `NOTES.md`) — печатает запись для заметок в
  отчёте, её пишет проход интеграции. В единице работы `feature` batch не архивирует фичу — это делает
  проход проверки; в `task` проход интеграции гоняет тесты и пишет доки. Конфиг `parallel`
  (`enabled`, `max_workers: 2`), флаг `--max-parallel`, CI-параметр `max_parallel`; нужен чистый
  worktree и git auto-commit; `--task` всегда последовательный. Ошибочная группа останавливает прогон.
- `context.rules_file` (по умолчанию `CLAUDE.md`) — какой файл правил встраивать в промпт; проект со
  сводом правил в `AGENTS.md` меняет одну строку конфига, а не код оркестратора.
- Тесты оркестратора `ai-flow/tests/` (парсер, выбор batch-а, промпты единиц работы, `NOTES.md`,
  непроверенные фичи, batch в реальном git с фейковым агентом) — добавлены в манифест `init.py`.
- Документация: `CLAUDE.md`, `PROMT_TASKS.md` §3/§7/§9/§10, `PROMT_AGENT.md`, `PROMT_CI.md`,
  `tasks/README.md`, `README.md`, память `task-completion` / `suggested-commands`, агенты
  `task-author` / `task-runner`, скилл `run-tasks`, `agents.yml`, workflow `ai-flow-tasks.yml`.
- Проверено: `python -m unittest discover -s ai-flow/tests` (36 тестов); фейковый агент в scratch-репо —
  batch из двух задач → проход интеграции пишет `NOTES.md` → задача с зависимостями через перенос,
  скобки и `.md` → проход проверки → архив фичи; worktree убраны, воркер, тронувший общий файл,
  валит batch без изменений в основной ветке.
